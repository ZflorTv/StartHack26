...
export class RiskProfileScreen {
  ...
  render(): void {
    const profiles = Object.entries(RISK_PROFILES).map(([key, profile]) => {
      const riskProfile = key as RiskProfile
      const displayAllocation = this.getDisplayAllocation(profile.suggestedAllocation)

      return `
      <div class="card card-hover risk-card" data-profile="${key}" style="cursor: pointer;">
        <div class="risk-card-icon">
          <img src="${this.iconByProfile[riskProfile]}" alt="${profile.name} icon" />
        </div>
        ...
        <div style="margin-top: var(--space-md);">
          ...
          <div style="display: flex; justify-content: space-between; margin-top: 4px;">
            ${Object.entries(displayAllocation)
              .filter(([_, v]) => v > 0)
              .map(([cat, v]) => `<span class="text-small text-muted">${this.formatCategoryLabel(cat)} ${v}%</span>`)
              .join('')}
          </div>
        </div>
      </div>
